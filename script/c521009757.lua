--キメラ・ハイドライブ・ドラグリッド・ダクネス
--Chimera Hydradrive Draghead - Darkness
--Scripted by TriDung
...
function s.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.GetMatchingGroup(s.rmfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,c)
	local gun=c:GetLinkedCardsCustom()
	if chk==0 then return Duel.IsExistingMatchingCard(s.rmfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,gun+c) end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,g-gun,#(g-gun),0,0)
end
